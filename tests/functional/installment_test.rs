...
fn should_retrieve_installments() {
    ...
    assert_eq!(Some(&String::from("554960")), installment_details[0].bin_number());
    assert_eq!(Some(&BigDecimal::from_str("100").unwrap()), installment_details[0].price());
    assert_eq!(Some(&String::from("CREDIT_CARD")), installment_details[0].card_type());
    assert_eq!(Some(&String::from("MASTER_CARD")), installment_details[0].card_association());
    assert_eq!(Some(&String::from("Bonus")), installment_details[0].card_family_name());
    assert_ne!(None, installment_details[0].installment_prices());
    assert!(installment_prices[0].installment_number().unwrap() > &0);
    assert!(installment_prices[0].installment_price().unwrap().gt(&BigDecimal::zero()));
    assert!(installment_prices[0].total_price().unwrap().gt(&BigDecimal::zero()));
