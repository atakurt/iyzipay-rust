...
fn should_initialize_checkout_form_for_standard_merchant() {
    ...
    let basket_items: Vec<BasketItem> = vec![BasketItemBuilder::create().price(BigDecimal::from_str("0.3").unwrap()).build()];

    let request: CreateCheckoutFormInitializeRequest = CreateCheckoutFormInitializeRequestBuilder::create()
    ...
    let checkout_form_initialize: CheckoutFormInitialize = CheckoutFormInitialize::create(&request, &get_test_options()).unwrap();
...
fn should_retrieve_checkout_form_result() {
    ...
    let retrieve_checkout_form_request: RetrieveCheckoutFormRequest = RetrieveCheckoutFormRequestBuilder::create()
        .token(checkout_form_initialize.token().unwrap().to_owned())
        .build();

    let checkout_form: CheckoutForm = CheckoutForm::retrieve(&retrieve_checkout_form_request, &get_test_options()).unwrap();

    debug!("{:?}", checkout_form);

    assert_ne!(None, checkout_form.error_message());
    assert_eq!(Some(&Status::Failure.value().to_string()), checkout_form.status());
