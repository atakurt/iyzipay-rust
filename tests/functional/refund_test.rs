...
fn should_refund_payment() {
    ...
    request.set_payment_transaction_id(payment.payment_items().unwrap()[0].payment_transaction_id().unwrap().to_owned());
    ...
    assert_eq!(payment.payment_items().unwrap()[0].payment_transaction_id(), refund.payment_transaction_id());
